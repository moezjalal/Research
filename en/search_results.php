...
<nav class="navbar navbar-expand-lg navbar-light bg-body-tertiary fixed-top">
  ...
  <img src="../img/h.jpg" alt="Logo" style="width: 280px; height: auto;">
    ...
    <div class="card-img-overlay">
        <p class="card-text text-white">في هذي الصفحه  يمكن البحث عن القرارات</p> <!-- النص باللون الأبيض -->
    </div>
</div>
...
<div class="container py-5">
    ...
    <div class="row">
    <?php
include '../db__admin.php'; // الاتصال بقاعدة البيانات
...
// الحصول على مدخل البحث
$search = isset($_GET['query']) ? $conn->real_escape_string($_GET['query']) : '';

// التحقق من وجود قيمة البحث
if (!empty($search)) {
    // تعديل الاستعلام لإضافة البحث عن النتائج المشابهة
    $sql = "SELECT id, title, description, date, nambar 
            FROM decisionsen
    ...
    if ($result && $result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<td><a href="decision_detail.php?id=' . $row["id"] . '" class="btn btn-primary btn-sm"style="style="background-color: #28a745; border: none;">View More</a></td>';
            echo '</tr>';
        }
        ...
        echo "لا توجد نتائج مطابقة. حاول البحث بكلمات أخرى.";
    }
} else {
    echo "يرجى إدخال كلمة للبحث.";
}
